...
class BuildingMetricsCalculator:
    ...
    def calculate_spatial_metrics(self):
        ...
        self.tessellation['num_neighbors'] = momepy.neighbors(self.tessellation, queen_tess) #should be in tessellation metricss
    ...
    def calculate_tessellation_metrics(self):
        ...
        self.tessellation['area'] = self.tessellation.geometry.area
        # self.tessellation['num_neighbors'] = momepy.neighbors(self.tessellation, queen_tess) it'd be nice to have it here
        ...
        self.tessellation['CAR'] = building_areas / self.tessellation['area']
...
#     def calculate_tessellation_metrics(self):
#         """
#         Calculate tessellation-related metrics and attach relevant attributes.
#         This method computes tessellation metrics (e.g., number of neighbors, compactness,
#         area, and Covered Area Ratio (CAR)) and ensures that these fields exist in the tessellation
#         GeoDataFrame before it is merged with the buildings.
#         """
#         # Build the contiguity graph for tessellation
#         queen_tess = graph.Graph.build_contiguity(self.tessellation, rook=False)

#         # Calculate number of neighbors for each tessellation cell
#         self.tessellation['num_neighbors'] = momepy.neighbors(self.tessellation, queen_tess)

#         # Calculate compactness weighted axis for tessellation
#         self.tessellation['cwa'] = momepy.compactness_weighted_axis(self.tessellation)

#         # Calculate tessellation area and rename it to 'tes_area'
#         self.tessellation['tes_area'] = self.tessellation.geometry.area

#         # Ensure tessellation has a unique ID for each cell.
#         if 'uID' not in self.tessellation.columns:
#             self.tessellation['uID'] = range(len(self.tessellation))

#         # Next, assign each building a tessellation cell ID using a spatial join.
#         # This will allow us to group buildings by tessellation cell.
#         buildings_with_tess = gpd.sjoin(self.clean_buildings, self.tessellation[['uID', 'tes_area']],
#                                         how='left', predicate='within')

#         # Group buildings by tessellation cell (using the tessellation uID) and sum their areas.
#         building_areas = buildings_with_tess.groupby('uID')['area'].sum()

#         # Set the tessellation index to uID so we can merge the data.
#         self.tessellation = self.tessellation.set_index('uID')

#         # Calculate CAR: Covered Area Ratio = (sum of building areas) / (tessellation cell area)
#         self.tessellation['CAR'] = building_areas / self.tessellation['tes_area']

#         # Reset index so uID becomes a column again.
#         self.tessellation = self.tessellation.reset_index()

#         # Finally, calculate cell alignment between building and tessellation orientations.
#         blg_orient = momepy.orientation(self.clean_buildings)
#         tess_orient = momepy.orientation(self.tessellation)
#         self.clean_buildings['cell_alignment'] = momepy.cell_alignment(blg_orient, tess_orient)
...
#     def calculate_topology_metrics(self):
#         """
#         Calculate topology-related metrics and add centroids directly to buildings.
#         Also computes average inter-building distances based on Delaunay triangulation.
#         """
#         # Compute centroids and add as a new column.
#         self.clean_buildings['centroid'] = self.clean_buildings.geometry.apply(
#             lambda geom: geom.convex_hull.centroid if geom.geom_type == 'MultiPolygon' else geom.centroid
#         )

#         # Calculate Delaunay triangulation and average distances.
#         coordinates = np.column_stack((
#             self.clean_buildings['centroid'].x,
#             self.clean_buildings['centroid'].y
#         ))

#         tri = Delaunay(coordinates)
#         G = self._create_delaunay_graph(coordinates, tri)
#         self.clean_buildings['avg_distance'] = self._calculate_avg_distance_for_buildings(G)
#         self.weighted_edges = self._calculate_weighted_edges(G, coordinates)
...
#     def _calculate_avg_distance_for_buildings(self, G):
#         """
#         Calculate average weighted distances for each building (node) from the Delaunay graph.
#         Returns a pandas Series with the average distance for each building.
#         """
#         avg_distances = []
#         for node in range(len(self.clean_buildings)):
#             neighbors = list(G.neighbors(node))
#             if neighbors:
#                 weights = [G[node][neighbor]['weight'] for neighbor in neighbors]
...
#         return pd.Series(avg_distances, index=self.clean_buildings.index)

#     def finalize_buildings(self):
#         """
#         Merge tessellation data with the buildings using a spatial join to include tessellation attributes.
#         This returns a complete GeoDataFrame that includes building metrics, the computed centroids, and tessellation data.
#         """
#         complete_buildings = gpd.sjoin(self.clean_buildings, self.tessellation, how='left', predicate='intersects')

#         # Remove auxiliary column from spatial join if present.
#         if 'index_right' in complete_buildings.columns:
#             complete_buildings = complete_buildings.drop(columns=['index_right'])

#         return complete_buildings

#     def run_all_calculations(self):
#         """
#         Run all metric calculations and return the final buildings GeoDataFrame.

#         Returns:
#             GeoDataFrame: The complete buildings GeoDataFrame including centroids and tessellation attributes.
#         """
#         self.calculate_basic_metrics()
#         self.calculate_spatial_metrics()
#         self.calculate_shape_metrics()
#         self.calculate_tessellation_metrics()
#         self.calculate_topology_metrics()
#         return self.finalize_buildings()

#     def save_results(self):
#         """Save the updated GeoDataFrames."""
#         complete_buildings = self.finalize_buildings()
#         complete_buildings.to_file(f"{self.output_dir}/complete_buildings_with_metrics.shp")
#         self.tessellation.to_file(f"{self.output_dir}/tessellation_with_metrics.shp")
